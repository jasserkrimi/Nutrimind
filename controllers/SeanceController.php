...
class SeanceController {
    ...
    public function create() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $activer_notification = isset($_POST['activer_notification']) && $_POST['activer_notification'] == '1';

            if (!empty($activite_id) && !empty($jour) && !empty($heure)) {
                // Définir le statut selon si la notification est activée
                $statut = $activer_notification ? 'Programme' : 'Sans notification';
                
                // Insérer la séance
                $stmt = $this->db->prepare("INSERT INTO seances_programmees (activite_id, jour_semaine, heure_debut, statut) VALUES (:act, :jour, :heure, :statut)");
                $stmt->execute([
                    'act' => $activite_id, 
                    'jour' => $jour, 
                    'heure' => $heure,
